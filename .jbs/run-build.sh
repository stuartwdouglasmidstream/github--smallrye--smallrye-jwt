...
if [ -n "" ]; then
  echo "Setting version to 4.5.1 to match enforced version"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="4.5.1" | tee /root/project/logs/enforce-version.log
fi
